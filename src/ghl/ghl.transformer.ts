...
import { formatPhoneNumber, extractPhoneNumberFromVCard } from "../utils/format";
	...
	toPlatformMessage(webhook: EvolutionWebhook): GhlPlatformMessage {
		...
		if (webhook.type === "message") {
			...
			switch (msgData.type) {
				...
				case "document":
				case "audio":
					messageText = msgData.caption || `Received a ${msgData.type}`;
					if (msgData.url) {
						attachments.push({
							url: msgData.url,
					...
					break;
				 case "sticker":
					messageText = msgData.caption || "🟡 Received a sticker";
					if (msgData.url) {
						attachments.push({
							url: msgData.url,
