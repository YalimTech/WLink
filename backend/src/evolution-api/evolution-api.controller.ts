...
export class EvolutionApiController {
    ...
    const refreshed = await Promise.all(
      instances.map(async (instance) => {
        try {
          ...
          this.logger.log(
            `[getInstances] Estado obtenido para la instancia '${instance.instanceName}' (ID de BD: ${instance.id}): ${state}`,
          ); // CAMBIO: Usar instance.instanceName en el log
          ...
          if (state && state !== instance.state) {
            ...
            if (updatedInstance) {
              ...
              this.logger.log(
                `[getInstances] BD actualizada para la instancia '${instance.instanceName}'. Nuevo estado: ${state}`,
              ); // CAMBIO: Usar instance.instanceName en el log
            } else {
              this.logger.warn(
                `[getInstances] No se pudo actualizar el estado de la instancia '${instance.instanceName}' en la BD, a pesar de que la API de Evolution devolvió un nuevo estado.`,
              );
            }
          }
        } catch (err: any) {
          this.logger.warn(
            `[getInstances] Error al actualizar el estado de la instancia '${instance.instanceName}' (ID de BD: ${instance.id}): ${err.message}`,
          ); // CAMBIO: Usar instance.instanceName en el log
  ...
  async createInstance(@Req() req: AuthReq, @Body() dto: CreateInstanceDto) {
    ...
    if (!dto.instanceName || !dto.token) {
      // customName es opcional
      throw new HttpException(
        "Evolution API Instance Name and API Token are required.",
    ...
    try {
      ...
      this.logger.error(
        `Failed to create instance ${dto.instanceName} (Custom Name: ${dto.customName}): ${err.message}`,
      ); // CAMBIO: Logs
  ...
  async logoutInstance(@Param("id") id: string, @Req() req: AuthReq) {
    ...
    try {
      ...
      this.logger.log(
        `Instancia ${inst.instanceName} (ID de BD: ${inst.id}) desconectada exitosamente.`,
      ); // CAMBIO: Usar instanceName en el log
      return { success: true, message: "Logout command sent successfully." };
    } catch (err: any) {
      this.logger.error(
        `Error al desconectar ${inst.instanceName} (ID de BD: ${inst.id}): ${err.message}`,
      ); // CAMBIO: Usar instanceName en el log
      if (err instanceof HttpException) throw err;
      throw new HttpException(
        "Failed to logout instance",
        HttpStatus.INTERNAL_SERVER_ERROR,
      );
    }
  }
  ...
  async deleteInstance(@Param("id") id: string, @Req() req: AuthReq) {
    const { locationId } = req;
    this.logger.log(
      `Intentando eliminar la instancia con ID de BD: ${id} para la ubicación: ${locationId}`,
    );
    ...
    if (!instanceData || instanceData.locationId !== locationId) {
      throw new UnauthorizedException(
        "Instancia no encontrada o no autorizada para esta ubicación",
      );
    }
    ...
    this.logger.log(
      `Instancia con ID de BD ${id} eliminada de la base de datos local.`,
    );
    return {
      success: true,
  ...
  async updateInstance(
    ...
    if (!instanceData || instanceData.locationId !== locationId) {
      throw new HttpException(
        "Instance not found or not authorized for this location",
        HttpStatus.FORBIDDEN,
      );
    }
    try {
      // Usar el instanceName de Evolution API para la actualización en Prisma
      const updatedInstance = await this.prisma.updateInstanceCustomName(
        instanceData.instanceName,
        dto.customName,
      ); // CAMBIO: Llamar a updateInstanceCustomName con instanceName
      this.logger.log(
        `Nombre personalizado de la instancia ${instanceData.instanceName} actualizado a ${dto.customName}.`,
      ...
      this.logger.error(
        `Failed to update custom name for instance ${instanceData.instanceName}: ${err.message}`,
      ); // CAMBIO: Usar instanceName
      if (err instanceof HttpException) throw err;
      throw new HttpException(
        "Failed to update instance custom name",
        HttpStatus.INTERNAL_SERVER_ERROR,
      );
    }
  }
