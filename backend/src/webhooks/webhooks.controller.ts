...
export class WebhooksController {
  ...
  async handleGhlSendMessage(
    ...
    this.logger.debug(
      `[WebhooksController] Received GHL Send-Message for location ${locationId}. Payload: ${JSON.stringify(ghlWebhook)}`,
    );
    res.status(HttpStatus.OK).send("Webhook received");
    ...
    try {
      ...
      if (!conversationProviderId) {
        this.logger.warn(
          `[WebhooksController] Wrong conversation provider ID. Ignoring send-message webhook.`,
        );
        return;
      }
      if (!locationId) {
        throw new BadRequestException(
          "Location ID is missing from send-message webhook.",
        );
      }
      ...
      this.logger.log(
        `[WebhooksController] GHL send-message processed successfully for location ${locationId}, Message ID: ${messageId}`,
      );
    } catch (error) {
      this.logger.error(
        `[WebhooksController] Error processing GHL send-message for location ${locationId}: ${error.message}. Stack: ${error.stack}`,
      );
      if (locationId && messageId) {
        await this.evolutionApiService.updateGhlMessageStatus(
          locationId,
